...
#WINDOW SIZES TO NORMALISE DIVERSITY STATS 
NCXG_window_sizes <- read.csv("indata/genome_files/window_sizes/NCXG_window_sizes.csv")
FZD_WS_files <- list.files("indata/genome_files/window_sizes/", pattern = '*ZFD*')
FZD_window_sizes <- lapply(FZD_WS_files, function(x) read.table(paste0("indata/genome_files/window_sizes/", x), header = T)) %>% 
  bind_rows()

#WHETHER WE'RE USING THE SAME NUMBER OF ZS OR NOT, OTHER OPTION IS 8As_8Bs_PH
vcf_type = 'correctedZ_PH'
...
ZvsA_plot <- PG_data %>% 
  filter(SNP_type %in% c('8As8BsPHNCXG', 'CorZPHNCXG')) %>% 
  dplyr::select(cols) %>% 
...
system(paste0("open plots/", vcf_type, "_ZvsA.png"))

Summary_tables <- list()
make_plots = T
mss = 0

for (snptype in c("CorZPHNCXG", "CorZPHFD", "CorZPHZD")){
  All_Chroms <- PG_data %>% 
    filter(SNP_type == snptype) %>% 
    filter(!is.na(A_n.segregating.sites) & !is.na(B_n.segregating.sites))
  if (snptype == 'CorZPHNCXG'){
    All_Chroms <- merge(All_Chroms, NCXG_window_sizes, by = c("contig", "Pos"), all.y = F) %>% 
      rename(adj.n.sites = length) %>% 
      filter(adj.n.sites > 0)
  }else if (snptype == 'CorZPHFD'){
    All_Chroms <- merge(All_Chroms, FZD_window_sizes[,-5], by = c("contig", "Pos"), all.y = F) %>% 
      rename(adj.n.sites = FD_length) %>% 
      filter(adj.n.sites > 0)
  } else if (snptype == 'CorZPHZD'){
    All_Chroms <- merge(All_Chroms, FZD_window_sizes[-4], by = c("contig", "Pos"), all.y = F) %>% 
      rename(adj.n.sites = ZD_length) %>% 
      filter(adj.n.sites > 0)
  }
  ...
  if (make_plots == T){
   ...
   All_Chroms[low_seg_sites,c("FST", "dxy")] <- NA
    ...
    All_Chroms$Dxy_Adj <- All_Chroms$dxy / All_Chroms$adj.n.sites
    All_Chroms$Pi_All_Adj <- All_Chroms$dxy / All_Chroms$adj.n.sites
    ...
    MeanPi_Inversion_A <- All_Chroms %>%
      filter(Chr == "Z", Position > 6500000 & Position < 70100000) %>%
      summarise(mean=mean(Pi_A/adj.n.sites, na.rm = T), sd=sd(Pi_A/adj.n.sites, na.rm = T), n= n()) %>%
      mutate(se = sd/sqrt(n)) %>% 
    ...
    MeanPi_Inversion_B <- All_Chroms %>%
      filter(Chr == "Z", Position > 6500000 & Position < 70100000) %>%
      summarise(mean=mean(Pi_B/adj.n.sites, na.rm = T), sd=sd(Pi_B/adj.n.sites, na.rm = T), n= n()) %>%
      mutate(se = sd/sqrt(n)) %>% 
