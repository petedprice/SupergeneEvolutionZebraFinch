...
data <- read.table("outdata/work/JON_DEGDATA.txt")

hist(data$reads)
ggplot(data, aes(x = reads, y = Ds_cont)) + geom_point()
...
model4 <- lmer(reads ~ (Ds_cont * genotype)+ (1|sample) + (1|gene), data)
...
############## ASE 
data2 <- read.table("outdata/work/JON_ASEDATA.txt")
...
simulationOutput4 <- simulateResiduals(ASEmodel4)
plot(simulationOutput4)

data2$scaled_Ds_cont <- scale(data2$Ds_cont)
ASEmodel4 <- glmer(cbind(aCount, totalCount) ~  genotype +
                     (1|sample) + (1|gene), data2, family = "binomial")
